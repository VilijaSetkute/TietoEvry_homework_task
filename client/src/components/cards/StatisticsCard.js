...
import {faHeart, faUtensils, faCircle, faCircleHalfStroke, faCircleNotch} from "@fortawesome/free-solid-svg-icons";
import http from "../../plugins/http";

const StatisticsCard = ({id, recipe, setCalsStatistics}) => {

    const {favorites, setFavorites, planned, setPlanned, weightLogs} = useContext(mainContext)
        ...
        <div className='recipe-card__horizontal'>
            <div className='d-flex justify-content-between'>
                ...
                <div className='position-relative d-flex'>
                    ...
                    <div className='position-relative'>
                        <FontAwesomeIcon className={`${recipe.recipe.quarter > 0 ? 'text-success opacity-25' : 'text-success pointer'} fs-4 m-1 p-2 rounded-circle`}
                                         icon={faUtensils}
            ...
            <div className='card-data'>
                ...
                <div className='block-margin'>
                    <div className='d-flex align-items-center'>
                        <div className='progress-name-width'>Fats: {Math.ceil(recipe.recipe.digest[0].total)} g.</div>
                    ...
                    <div className='d-flex align-items-center'>
                        <div className='progress-name-width'>Carbs: {Math.ceil(recipe.recipe.digest[1].total)} g.</div>
                        <div className="progress ms-5 progress-bar-width">
                    ...
                    <div className='d-flex align-items-center'>
                        <div className='progress-name-width'>Proteins: {Math.ceil(recipe.recipe.digest[2].total)} g.</div>
                        <div className="progress ms-5 progress-bar-width">
                ...
                <div className='block-margin'>
                    ...
                    <div className='d-flex text-secondary'>
                        <div className={`position-relative ${recipe.recipe.quarter > 0 ? 'text-success' : 'undefined'}`} onClick={() => mealQuarter(1)}>
                            <FontAwesomeIcon icon={faCircleHalfStroke} className='pointer fs-3 m-2 bg-white rotate180'/>
                            <FontAwesomeIcon icon={faCircleHalfStroke} className='pointer position-absolute text-white fs-3 m-2 rotate270'/>
                        ...
                        <FontAwesomeIcon icon={faCircleHalfStroke} className={`pointer fs-3 m-2 rotate180 ${recipe.recipe.quarter > 1 ? 'text-success' : 'undefined'}`}  onClick={() => mealQuarter(2)}/>
                        <div className={`position-relative ${recipe.recipe.quarter > 2 ? 'text-success' : 'undefined'}`} onClick={() => mealQuarter(3)}>
                        ...
                        <FontAwesomeIcon icon={faCircle} className={`pointer fs-3 m-2 ${recipe.recipe.quarter > 3 ? 'text-success' : 'undefined'}`} onClick={() => mealQuarter(4)}/>
                    </div>
                </div>
            </div>
        </div>
